"""
Fetch and validate live phishing URLs from OpenPhish.

OpenPhish provides a community feed of active phishing URLs.
This script fetches from their public feed and validates URLs are truly live.
"""
import requests
import socket
import time
...
def fetch_openphish_urls(count: int = 10000) -> List[str]:
    """Fetch OpenPhish URLs from their public feed."""
    print(f"[OpenPhish] Fetching up to {count} URLs from public feed...")
    ...
    try:
        ...
        print(f"[OpenPhish] Fetched {len(urls)} URLs from feed")
        return urls

    except Exception as e:
        print(f"[OpenPhish] Error fetching feed: {e}")
        return []


def main():
    """Find 10 live OpenPhish URLs."""
    import warnings
    warnings.filterwarnings('ignore', message='Unverified HTTPS request')

    print("=" * 100)
    print("FINDING LIVE OPENPHISH PHISHING URLs")
    print("=" * 100)
    print("\nTarget: 10 live URLs with ≤20% numeric content")
    print("Strategy: Fetch from OpenPhish feed, randomly sample for validation")
    print("⚠️  WARNING: Validating potentially malicious URLs\n")

    # Fetch URLs from OpenPhish
    all_urls = fetch_openphish_urls(count=10000)

    if not all_urls:
        print("\n⚠️  Failed to fetch URLs from OpenPhish")
        return
    ...
    if len(live_urls) >= 10:
        print("\n" + "=" * 100)
        print("10 VERIFIED LIVE PHISHING URLs (OpenPhish)")
        print("=" * 100)
        ...
        # Save results
        output_file = "live_openphish_validated.json"
